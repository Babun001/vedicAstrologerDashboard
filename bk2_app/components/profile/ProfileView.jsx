...
export default function ProfileView({ astrologer, onUpdate }) {
  ...
  const [form, setForm] = useState({
    ...
    bio: astrologer?.bio || "",
    reportRate: astrologer?.reportRate ?? "",
    questionRate1: astrologer?.questionRate1 ?? "",
    questionRate2: astrologer?.questionRate2 ?? "",
    questionRate3: astrologer?.questionRate3 ?? "",
  });
    ...
    try {
      const payload = {
        ...
        languages: form.languages
          ...
          .filter(Boolean),
        bio: form.bio.trim(),
        ...(form.reportRate !== "" && { reportRate: Number(form.reportRate) }),
        ...(form.questionRate1 !== "" && { questionRate1: Number(form.questionRate1) }),
        ...(form.questionRate2 !== "" && { questionRate2: Number(form.questionRate2) }),
        ...(form.questionRate3 !== "" && { questionRate3: Number(form.questionRate3) }),
      };
  ...
  return (
    <>
      ...
      <div className="cr-card" style={{ padding: 20 }}>
        ...
        <label className="cr-field-label" style={{ marginTop: 4 }}>
          Your Rates (₹)
        </label>
        <div className="cr-field-row">
          <div className="cr-field-col">
            <label className="cr-field-label">Per Report</label>
            <div className="cr-field">
              <IndianRupee size={15} color="#9C8A6A" />
              <input
                type="number"
                min="0"
                value={form.reportRate}
                onChange={update("reportRate")}
                placeholder="300"
              />
            </div>
          </div>
          <div className="cr-field-col">
            <label className="cr-field-label">1 Question Answer</label>
            <div className="cr-field">
              <IndianRupee size={15} color="#9C8A6A" />
              <input
                type="number"
                min="0"
                value={form.questionRate1}
                onChange={update("questionRate1")}
                placeholder="100"
              />
            </div>
          </div>
        ...
        <div className="cr-field-row">
          <div className="cr-field-col">
            <label className="cr-field-label">2 Questions Answer</label>
            <div className="cr-field">
              <IndianRupee size={15} color="#9C8A6A" />
              <input
                type="number"
                min="0"
                value={form.questionRate2}
                onChange={update("questionRate2")}
                placeholder="180"
              />
            </div>
          </div>
          <div className="cr-field-col">
            <label className="cr-field-label">3 Questions Answer</label>
            <div className="cr-field">
              <IndianRupee size={15} color="#9C8A6A" />
              <input
                type="number"
                min="0"
                value={form.questionRate3}
                onChange={update("questionRate3")}
                placeholder="250"
              />
            </div>
          </div>
